...
class Wizard(object):
    ...
    def get_pony(self, pony_id):
        try:
            endpoint = "https://www.forgottenrunes.com/api/art/{}/{}/riding/pony/{}".format(
                self.slug, self.token_id, pony_id)
    ...
    def name(self):
        readme = open(
            "{}/{}/README.md".format(self.artwork_root, self.token_id), 'rt')
    ...
    def spritesheet_familiar_rows(self):
        path, dirs, files = next(
            os.walk("{}/50/familiar-spritesheet/frames".format(self.path)))
        return len(files) / 4
...
class WizardFactory:
    ...
    def get_pony_walkcycle(token_id, pony_id, is_soul=False):
        url = "https://www.forgottenrunes.com/api/art/{}/{}/riding/pony/{}.gif".format(
            "wizards" if not is_soul else "souls", token_id, pony_id)
    ...
    def catchphrase(token_id, phrase, is_soul=False, is_warrior=False):
        wizard = WizardFactory.get_wizard(
            token_id, is_soul=is_soul, is_warrior=is_warrior)
    ...
    def anatomy(token_id, is_soul=False, is_warrior=False):
        ...
        for i in range(25):
            ...
            other = WizardFactory.random_cached_wizard(
                is_soul=is_soul, is_warrior=is_warrior, ignore=ignore)
            ...
            others.append(other if other is not None else WizardFactory.get_wizard(
                random_id(), is_soul=is_soul, is_warrior=is_warrior))
        img = imagetools.poster(
            wizard, others, is_warrior=is_warrior, is_soul=is_soul)
        return (wizard, imagetools.to_png(img))

    @staticmethod
    def random_cached_wizard(is_soul=False, is_warrior=False, ignore=[]):
        path_artwork = "{}/artwork/{}".format(
            os.getcwd(), "souls" if is_soul else "warriors" if is_warrior else "wizards")
        available = [f for f in sorted(os.listdir(path_artwork)) if not f.startswith(
            '.') and int(f) not in ignore]
        if len(available) > 0:
    ...
    def get_wizard(token_id, refresh=False, is_soul=False, is_warrior=False):
        ...
        wizard = Wizard(token_id, path_artwork,
                        60 if is_warrior else 50, is_soul, is_warrior)
        ...
        def download_content():
            ...
            if cached_wizard_path is None:
                ...
                endpoint_wizard_artwork = 'https://www.forgottenrunes.com/api/art/{}/{}.zip'.format(
                    "souls" if is_soul else "warriors" if is_warrior else "wizards", token_id)
                urllib.request.urlretrieve(
                    endpoint_wizard_artwork.format(token_id), zip_file)
            ...
            if not os.path.isfile(wizard.pfp) or refresh:
                pfp_original_file = "{}-{}.png".format(wizard.token_id, wizard.name.replace(
                    "  ", " ").replace(" ", "-").replace("'", "").replace(".", ""))
                pfp_original = "{}/{}/{}".format(wizard.path,
                                                 wizard.base_dimension*8, pfp_original_file)
                shutil.copy(pfp_original, wizard.pfp)
            if not os.path.isfile(wizard.pfp_nobg) or refresh:
                pfp_original_file = "{}-{}-nobg.png".format(wizard.token_id, wizard.name.replace(
                    "  ", " ").replace(" ", "-").replace("'", "").replace(".", ""))
                pfp_original = "{}/{}/{}".format(wizard.path,
                                                 wizard.base_dimension*8, pfp_original_file)
...
async def summon_wizards(wizards, is_soul=False, is_warrior=False):
    ...
    for wizard in wizards:
        ...
        WizardFactory.get_wizard(wizard, refresh=True,
                                 is_soul=is_soul, is_warrior=is_warrior)
